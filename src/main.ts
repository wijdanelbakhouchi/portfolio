...
import { initContactForm } from './contact.ts';
...
matchMedia('(min-width: 1051px)').addEventListener('change', () => setMenu(false));
...
// Active section observer for navigation highlights and scroll reveal
...
function revealHashProject() {
  ...
  if (target?.matches('.project[hidden]')) {
