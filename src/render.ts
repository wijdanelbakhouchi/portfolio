import { workstation, projectVisual } from './visuals.ts';
...
function projectCard(project: ProjectItem, index: number) {
  ...
  return `<article class="project${project.featured ? ' flagship' : project.spotlight ? ' spotlight' : ''}" id="${e(project.id)}" data-project-id="${e(project.id)}" data-category-slug="${categorySlug}">
    ${projectVisual(project.id)}
    ...
    <a class="text-link" href="${e(project.githubUrl)}" target="_blank" rel="noopener noreferrer">${githubIcon} GitHub <span aria-hidden="true">↗</span></a>
    ${project.demoUrl ? external(project.demoUrl, 'Live demo') : ''}</div>
  ...
  <section class="section contact-section" id="contact" data-section="contact" aria-labelledby="contact-title"><div class="wrap">
    ...
    <div class="contact-grid">
      <div class="contact-form-wrapper">
        <form class="contact-form" id="contact-form" action="/api/contact" method="post"><p class="form-intro">Send a message</p>
          <div class="form-row-dual">
            <div class="form-group">
              <label for="contact-name" class="form-label">Your name <span class="required" aria-hidden="true">*</span></label>
              <input type="text" id="contact-name" name="name" maxlength="100" aria-describedby="name-error" class="form-input" placeholder="Your full name" required autocomplete="name">
            ...
            <div class="form-group">
              ...
              <input type="email" id="contact-email" name="email" maxlength="254" aria-describedby="email-error" class="form-input" placeholder="you@example.com" required autocomplete="email">
          ...
          <div class="form-group">
            ...
            <input type="text" id="contact-subject" name="subject" maxlength="160" aria-describedby="subject-error" class="form-input" placeholder="What's on your mind?" required>
          ...
          <div class="form-group">
            ...
            <textarea id="contact-message" name="message" maxlength="5000" aria-describedby="message-error" class="form-input form-textarea" rows="5" placeholder="A little about your project or opportunity..." required></textarea>
            <span class="form-error" id="message-error" aria-live="polite"></span>
          </div>
          <div class="honeypot" aria-hidden="true"><label for="contact-website">Leave this field empty</label><input id="contact-website" name="website" tabindex="-1" autocomplete="off"></div><noscript><p>Please use the direct email link to get in touch; the form requires JavaScript.</p></noscript><div class="form-footer">
            <p class="form-helper">All fields are required.</p>
            <button type="submit" class="button primary form-submit" disabled>Send message <span aria-hidden="true">↗</span></button>
          </div>
      ...
      <aside class="contact-sidebar">
        <div class="contact-direct-card">
          ...
          <p class="meta contact-loc">${e(personalInfo.location)}</p>
        </div>
        <div class="contact-statement">
          <p class="eyebrow">Research &amp; Engineering</p>
          <blockquote class="closing-statement">Let’s talk about what’s next.</blockquote>
